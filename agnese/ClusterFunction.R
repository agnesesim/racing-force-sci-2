...
patents_tagged <- udpipe_annotate(ud_model, x = monitoring$abstract, doc_id = monitoring$patent_id) %>%
  as_tibble()
...
remove_word <- c("system", "data", "image", "method", "that", "vehicle","device", "images")

tmp_keywords <- stats %>%
  filter(freq > 10) %>%
  filter( !(keyword %in% remove_word)) %>%
...
# find the keywords
patents_keyword <- monitoring %>% 
  mutate(keys = str_match_all(abstract, keywords))


# matrice di incidenza per grafo
x <- integer(nrow(monitoring)*nrow(monitoring))
rown <- monitoring[["patent_id"]]
coln <- monitoring[["patent_id"]]
m <- matrix(x, nrow =nrow(monitoring), byrow = TRUE, 
            dimnames = list(rown, coln))

patents_left = monitoring[["patent_id"]]
for (p1 in monitoring[["patent_id"]]){
...
save(m, file = "agnese/matrix/monitoring_adiac_abstract.RData")
write.table(m, file="agnese/matrix/monitoring_abstract_correlation.txt", col.names = NA)

load(file="agnese/matrix/monitoring_adiac_abstract.RData")
...
tmp_m[tmp_m < 5] <- 0
net = network(tmp_m, directed = FALSE)
ggnet2(net, node.size = 6, node.color = "grey", edge.size = 1, color = clusters)
